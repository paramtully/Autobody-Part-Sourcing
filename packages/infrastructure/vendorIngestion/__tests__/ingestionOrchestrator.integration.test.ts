...
import type { UpsertListingResult } from '../ingestionRepository/consistencyResult';
...
class MockIngestionRepositories implements IngestionRepositories {
    ...
    async upsertListing(dto: CleanedDTO, action: 'INSERT' | 'UPDATE'): Promise<UpsertListingResult> {
        const key = `${dto.vendorId}:${dto.vendorListingExternalId}`;
        const existingListing = this.listings.get(key);

        const makeResult = (listingId: string): UpsertListingResult => ({
            listingId,
            partId: 'mock-part-id',
            consistencyChecks: [],
            entitiesCreated: {
